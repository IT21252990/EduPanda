...
const Register = () => {
  ...
  // Add state to manage OTP input
  const [otp, setOtp] = useState('');

  // Add state to manage whether to show OTP input
  const [showOTPInput, setShowOTPInput] = useState(false);

  // Add handleSubmitOTP function to submit OTP
  const handleSubmitOTP = async (e) => {
    e.preventDefault();

    try {
      const response = await fetch('http://localhost:5002/api/users/verify-otp', {
        method: 'POST',
        headers: {
